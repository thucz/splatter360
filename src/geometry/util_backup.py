...
class Equirec2Cube:
    ...
    def _xyzcube(self):
        ...
        # # Up face (y = 0.5)
        # self.xyz[:, 0 * self.face_w:1 * self.face_w, [0, 2]] = - self.grid[::-1, :]
        # self.xyz[:, 0 * self.face_w:1 * self.face_w, 1] = 0.5

        # # Back face (z = -0.5)
        # self.xyz[:, 1 * self.face_w:2 * self.face_w, [0, 1]] = self.grid[:, ::-1]
        # self.xyz[:, 1 * self.face_w:2 * self.face_w, 2] = -0.5

        # # Left face (x = -0.5)
        # self.xyz[:, 2 * self.face_w:3 * self.face_w, [2, 1]] = self.grid
        # self.xyz[:, 2 * self.face_w:3 * self.face_w, 0] = -0.5

        # # Front face (z = 0.5)
        # self.xyz[:, 3 * self.face_w:4 * self.face_w, [0, 1]] = self.grid
        # self.xyz[:, 3 * self.face_w:4 * self.face_w, 2] = 0.5

        # # Right face (x = 0.5)
        # self.xyz[:, 4 * self.face_w:5 * self.face_w, [2, 1]] = self.grid[:, ::-1]
        # self.xyz[:, 4 * self.face_w:5 * self.face_w, 0] = 0.5
