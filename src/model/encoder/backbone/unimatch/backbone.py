...
class CNNEncoder(nn.Module):
    def __init__(self, output_dim=128,
        ...
        if downscale_factor == 4 or downscale_factor == 8:
            self.layer1 = self._make_layer(feature_dims[0], stride=1, norm_layer=norm_layer)  # 1/2
            stride = 2 if downscale_factor == 8 else 1
        elif downscale_factor == 16:
            self.layer1 = self._make_layer(feature_dims[0], stride=2, norm_layer=norm_layer)  # 1/4
            stride = 2
        else:
            raise NotImplementedError           
        self.layer2 = self._make_layer(feature_dims[1], stride=2, norm_layer=norm_layer)  # 1/4 or 1/8
